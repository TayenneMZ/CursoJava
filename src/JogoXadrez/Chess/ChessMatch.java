...
public class ChessMatch {
    ...
    public ChessPiece[][] getPieces() {
        ...
        for (int linha = 0; linha < board.getRows(); linha++) {
            for (int coluna = 0; coluna < board.getColumns(); coluna++) {
                matriz[linha][coluna] = (ChessPiece) board.piece(linha, coluna);
            }
        }
    ...
    private ChessPiece king(Color color) {
        List<Piece> list = piecesOnTheBoard.stream().filter(peca -> ((ChessPiece) peca).getColor() == color).collect(Collectors.toList());
        for (Piece p : list) {
    ...
    private boolean testCheck(Color color) {
        ...
        List<Piece> opponentPieces = piecesOnTheBoard.stream().filter(peca -> ((ChessPiece) peca).getColor() == opponent(color)).collect(Collectors.toList());
